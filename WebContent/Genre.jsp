...
<head>
	...
	String genreHiddenID = request.getParameter("id");
	if (genreHiddenID == null) {
	...
	try {
		...
		PreparedStatement pstmtTitle = conn.prepareStatement("SELECT * FROM genre WHERE genreID = ?");
		pstmtTitle.setString(1, genreHiddenID);
		ResultSet rsTitle = pstmtTitle.executeQuery();
		rsTitle.next();
		String currentGenre = rsTitle.getString("genreName");
	%>

    <title><%=currentGenre%> | GameShop</title>
        ...
        <div class="row">
            ...
            <div class="col-md-9">
                ...
                <div class="row">
					...
					<%	
							PreparedStatement pstmtGames = conn.prepareStatement(
									"SELECT * " +
			            			"FROM  game gm, genre gr, game_genre gmgr " +
			            			"WHERE gm.gameID = gmgr.gameID " +
			            			"AND   gr.genreID = gmgr.genreID " +
			            			"AND   gr.genreID LIKE ? " +
			            			"ORDER BY gm.releaseDate DESC"
							);
							pstmtGames.setString(1, genreHiddenID);
										...
										"<div class='col-sm-4 col-lg-4 col-md-4'>" +
											"<div class='thumbnail'>" +
					                            ...
					                            "<div class='caption'>" +
					                                ...
					                                "<h4><a href='Game.jsp?id=" + gameID + "'>" + gameTitle + "</a></h4>" +
					                                "<p>" + description + "</p>" +
					                            "</div>" +
					                            "<div class='ratings'>" +
